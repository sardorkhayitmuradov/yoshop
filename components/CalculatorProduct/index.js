...
export const CalculatorProduct = ({incrementItem , decrementItem , quantity , wrapClass}) => {
  return (
    <div className={`flex items-center justify-between max-w-[157px] w-full ${wrapClass}`}>
        <Image src={MinusIcon} className='cursor-pointer py-[10px]' alt='minus icon' width={24} height={24} onClick={decrementItem} />
        ...
        <Image src={PlusIcon} className='cursor-pointer py-[10px]' alt='plus icon' width={24} height={24} onClick={incrementItem} />
    </div>
  )
}
