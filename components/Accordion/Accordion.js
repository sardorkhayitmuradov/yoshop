...
const AccordionWrapper = ({ title, imgColor, accordionClassName, accordionHeaderClassName, accordionBodyClassname, children , products }) => {
  const [isOpen ,setIsOpen] = useState(false);

  return (
    <>
      <Accordion transition={{ duration: "300ms", timingFunction: "cubic-bezier(0, 0, 0.2, 1)" }} className={`mb-[40px] ${accordionClassName ? accordionClassName : ""}`}>
        <AccordionItem>
          {({ open }) => (
            <>
              <div className='border border-solid border-[#E5E7EB]'>
              ...
              <div className={`mb-[70px] px-[20px] ${open ? 'block' : "hidden"} mt-[40px]`}>
                {
                  isOpen ? <Button className={'cursor-pointer mb-5 font-PoppinsBold max-w-[60px] w-full text-white text-[20px] flex justify-center items-center bg-[#FF588A] border-solid border-x border-y border-[#fff] py-[13px] rounded-[10px]'}
                  onClick={() => setIsOpen(false)}
                  >
                    <Image src={Cancel} alt="cancel-icon" width={30} height={30} />
                  </Button> : <Button className={'cursor-pointer mb-5 font-PoppinsBold max-w-[220px] w-full text-white text-[20px] flex justify-center items-center bg-[#FF588A] border-solid border-x border-y border-[#fff] py-[13px] rounded-[10px]'}
                  onClick={() => setIsOpen(true)}
                  >
                    <Image src={PlusWhite} alt="shop-icon" className="pr-2" width={35} height={35} />
                    Add equipment
                  </Button>
                }
                <ul>
                  {
                    isOpen ?
                      products?.map(product => {
                        return (
                            <li key={product.id} className='mb-[25px] justify-between max-w-[700px] w-full'>
                                <div className='flex items-center justify-between max-w-[800px] w-full pl-[100px]'>
                                    <div className='flex items-center  max-w-[290px] w-full'>
                                        ...
                                        <p className='text-[20px] leading-[180%] text-[#111827] ml-10'>{product.title}</p>
                                    </div>
